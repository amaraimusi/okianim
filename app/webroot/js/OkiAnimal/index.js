...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'oki_animal', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	// CBBXS-1023
	// 動物カテゴリリストJSON
	var anim_ctg_id_json = jQuery('#anim_ctg_id_json').val();
	var animCtgIdList = JSON.parse(anim_ctg_id_json);
	disFilData['anim_ctg_id'] ={'fil_type':'select','option':{'list':animCtgIdList}};
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#oki_animal_tbl',
		'ajax_url':'oki_animal/ajax_pwms',
			});
...
//	initCsvImportFu('oki_animal/csv_fu');
