...
/**
 * 動物カテゴリのCakePHPモデルクラス
 *
 * @date 2015-9-16 | 2018-8-30
 * @version 3.0.4
 *
 */
class AnimCtg extends AppModel {

	public $name='AnimCtg';
	
	// 関連付けるテーブル CBBXS-1040
	public $useTable = 'anim_ctgs';
	 ...
	 * 動物カテゴリエンティティを取得
	 *
	 * 動物カテゴリテーブルからidに紐づくエンティティを取得します。
	 *
	 * @param int $id 動物カテゴリID
	 * @return array 動物カテゴリエンティティ
	 */
	...
	private function dumpSql($option){
		...
		$option['table']=$dbo->fullTableName($this->AnimCtg);
		$option['alias']='AnimCtg';
		
		$query = $dbo->buildStatement($option,$this->AnimCtg);
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="AnimCtg.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_anim_ctg_name'])){
			$cnds[]="AnimCtg.anim_ctg_name LIKE '%{$kjs['kj_anim_ctg_name']}%'";
		}
		if(!empty($kjs['kj_note'])){
			$cnds[]="AnimCtg.note LIKE '%{$kjs['kj_note']}%'";
		}
		if(!empty($kjs['kj_sort_no']) || $kjs['kj_sort_no'] ==='0' || $kjs['kj_sort_no'] ===0){
			$cnds[]="AnimCtg.sort_no = {$kjs['kj_sort_no']}";
		}
		$kj_delete_flg = $kjs['kj_delete_flg'];
		if(!empty($kjs['kj_delete_flg']) || $kjs['kj_delete_flg'] ==='0' || $kjs['kj_delete_flg'] ===0){
			if($kjs['kj_delete_flg'] != -1){
			   $cnds[]="AnimCtg.delete_flg = {$kjs['kj_delete_flg']}";
			}
		}
		if(!empty($kjs['kj_update_user'])){
			$cnds[]="AnimCtg.update_user LIKE '%{$kjs['kj_update_user']}%'";
		}
		if(!empty($kjs['kj_ip_addr'])){
			$cnds[]="AnimCtg.ip_addr LIKE '%{$kjs['kj_ip_addr']}%'";
		}
		if(!empty($kjs['kj_created'])){
			$kj_created=$kjs['kj_created'].' 00:00:00';
			$cnds[]="AnimCtg.created >= '{$kj_created}'";
		}
		if(!empty($kjs['kj_modified'])){
			$kj_modified=$kjs['kj_modified'].' 00:00:00';
			$cnds[]="AnimCtg.modified >= '{$kj_modified}'";
		}
	 ...
	 * エンティティをDB保存
	 *
	 * 動物カテゴリエンティティを動物カテゴリテーブルに保存します。
	 *
	 * @param array $ent 動物カテゴリエンティティ
	 * @param array $option オプション
	 *  - form_type フォーム種別  new_inp:新規入力 , copy:複製 , edit:編集
	 *  - ni_tr_place 新規入力追加場所フラグ 0:末尾 , 1:先頭
	 * @return array 動物カテゴリエンティティ（saveメソッドのレスポンス）
	 */
