<?php
$this->CrudBase->init(array('model_name'=>'AnimCtg'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'AnimCtg/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'AnimCtg/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('AnimCtg', array('url' => true )); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'動物カテゴリ名前、動物カテゴリ日、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_anim_ctg_name','動物カテゴリ名');
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'anim_ctg/csv_download';
...
<!-- 一覧テーブル -->
<table id="anim_ctg_tbl" border="1"  class="table table-striped table-bordered table-condensed">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'anim_ctg_name');
...
<div id="ajax_crud_new_inp_form" class="panel panel-primary">
	...
	<table><tbody>
		...
		<!-- CBBXS-1006 -->
		<tr><td>動物カテゴリ名: </td><td>
			<input type="text" name="anim_ctg_name" class="valid" value=""  maxlength="255" title="255文字以内で入力してください" />
			<label class="text-danger" for="anim_ctg_name"></label>
		</td></tr>
