<?php
$this->CrudBase->init(array('model_name'=>'OkiAnimal'));
...
$jsList[] = 'OkiAnimal/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('OkiAnimal', array('url' => true )); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'沖縄動物名前、沖縄動物日、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_animal_name','動物名');
		$this->CrudBase->inputKjSelect($kjs,'kj_anim_ctg_id','動物カテゴリ',$animCtgIdList); 
		$this->CrudBase->inputKjText($kjs,'kj_place','場所');
		$this->CrudBase->inputKjText($kjs,'kj_img_fn','動物画像');
...
<?php 
	...
	$csv_dl_url = $this->html->webroot . 'oki_animal/csv_download';
...
<!-- 一覧テーブル -->
<table id="oki_animal_tbl" border="1"  class="table table-striped table-bordered table-condensed">
...
<div id="ajax_crud_new_inp_form" class="panel panel-primary">
	...
	<table><tbody>
		...
		<!-- CBBXS-1006 -->
		<tr><td>動物名: </td><td>
			<input type="text" name="animal_name" class="valid" value=""  maxlength="255" title="255文字以内で入力してください" />
			<label class="text-danger" for="animal_name"></label>
		</td></tr>

		<tr><td>動物カテゴリ: </td><td>
			<?php $this->CrudBase->selectX('anim_ctg_id',null,$animCtgIdList,null);?>
			<label class="text-danger" for="anim_ctg_id"></label>
		</td></tr>

		<tr><td>場所: </td><td>
			<input type="text" name="place" class="valid" value=""  maxlength="255" title="255文字以内で入力してください" />
			<label class="text-danger" for="place"></label>
		</td></tr>
...
<div id="ajax_crud_delete_form" class="panel panel-danger">
	...
	<table><tbody>
		...
		<tr><td>沖縄動物名: </td><td>
			<span class="oki_animal_name"></span>
		</td></tr>
...
<div style="display:none">
	...
	<!-- CBBXS-1022 -->
	<input id="anim_ctg_id_json" type="hidden" value='<?php echo $anim_ctg_id_json; ?>' />
