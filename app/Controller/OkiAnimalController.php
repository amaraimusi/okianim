 ...
 * 
 * @note
 * 沖縄動物画面では沖縄動物一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2015-9-16 | 2018-4-25 削除のバグを主末井
 * @version 3.0.1
 *
 */
class OkiAnimalController extends CrudBaseController {

	/// 名称コード
	public $name = 'OkiAnimal';
	
	/// 使用しているモデル
	public $uses = array('OkiAnimal','CrudBase');
	
	/// オリジナルヘルパーの登録
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='OkiAnimal.sort_no';
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('OkiAnimal');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->OkiAnimal->findData($crudBaseData);
		...
		// 動物カテゴリリスト
		$animCtgIdList = $this->OkiAnimal->getAnimCtgIdList();
		$anim_ctg_id_json = json_encode($animCtgIdList,JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS);
		$this->set(array('animCtgIdList' => $animCtgIdList,'anim_ctg_id_json' => $anim_ctg_id_json));
...
// 		$dbExp->test($dao);
	...
	public function detail() {
		...
		$res=$this->edit_before('OkiAnimal');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'沖縄動物・詳細',
				'ent'=>$ent,
		));
...
// 		}
		...
		// エンティティをDB保存
		$this->OkiAnimal->begin();
		$ent = $this->OkiAnimal->saveEntity($ent,$regParam);
		$this->OkiAnimal->commit();//コミット
...
//		if(empty($this->Auth->user())) return 'Error:login is needed.';// 認証中でなければエラー
		...
		$this->OkiAnimal->begin();
		if($eliminate_flg == 0){
			$ent = $this->OkiAnimal->saveEntity($ent,$regParam); // 更新
		}else{
			$this->OkiAnimal->delete($ent['id']); // 削除
		}
		$this->OkiAnimal->commit();//コミット
...
//		if(empty($this->Auth->user())) return 'Error:login is needed.';// 認証中でなければエラー
		...
		// データ保存
		$this->OkiAnimal->begin();
		$this->OkiAnimal->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->OkiAnimal->commit();
...
//		if(empty($this->Auth->user())) return 'Error:login is needed.';// 認証中でなければエラー
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('oki_animal_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('oki_animal_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_animal_name','def'=>null),
			array('name'=>'kj_anim_ctg_id','def'=>null),
			array('name'=>'kj_place','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_animal_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '動物名は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_anim_ctg_id' => array(
						'custom'=>array(
								'rule' => array( 'custom', '/^[-]?[0-9] ?$/' ),
								'message' => '動物カテゴリは整数を入力してください。',
								'allowEmpty' => true
						),
				),
				'kj_place'=> array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'OkiAnimal.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'animal_name'=>array(
					'name'=>'動物名',
					'row_order'=>'OkiAnimal.animal_name',
					'clm_show'=>1,
			),
			'anim_ctg_id'=>array(
					'name'=>'動物カテゴリ',
					'row_order'=>'OkiAnimal.anim_ctg_id',
					'clm_show'=>1,
			),
			'place'=>array(
					'name'=>'場所',
					'row_order'=>'OkiAnimal.place',
					'clm_show'=>1,
			),
			'img_fn'=>array(
					'name'=>'動物画像',
					'row_order'=>'OkiAnimal.img_fn',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
					'row_order'=>'OkiAnimal.note',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
