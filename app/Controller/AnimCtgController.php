 ...
 * 
 * @note
 * 動物カテゴリ画面では動物カテゴリ一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2015-9-16 | 2018-4-25 削除のバグを主末井
 * @version 3.0.1
 *
 */
class AnimCtgController extends CrudBaseController {

	/// 名称コード
	public $name = 'AnimCtg';
	
	/// 使用しているモデル
	public $uses = array('AnimCtg','CrudBase');
	
	/// オリジナルヘルパーの登録
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='AnimCtg.sort_no';
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('AnimCtg');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->AnimCtg->findData($crudBaseData);
...
// 		$dbExp->test($dao);
	...
	public function detail() {
		...
		$res=$this->edit_before('AnimCtg');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'動物カテゴリ・詳細',
				'ent'=>$ent,
		));
...
// 		}
		...
		// エンティティをDB保存
		$this->AnimCtg->begin();
		$ent = $this->AnimCtg->saveEntity($ent,$regParam);
		$this->AnimCtg->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->AnimCtg->begin();
		if($eliminate_flg == 0){
			$ent = $this->AnimCtg->saveEntity($ent,$regParam); // 更新
		}else{
			$this->AnimCtg->delete($ent['id']); // 削除
		}
		$this->AnimCtg->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->AnimCtg->begin();
		$this->AnimCtg->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->AnimCtg->commit();
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('anim_ctg_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('anim_ctg_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_anim_ctg_name','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_anim_ctg_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '動物カテゴリ名は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_note'=> array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'AnimCtg.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'anim_ctg_name'=>array(
					'name'=>'動物カテゴリ名',
					'row_order'=>'AnimCtg.anim_ctg_name',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
					'row_order'=>'AnimCtg.note',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'AnimCtg.sort_no',
					'clm_show'=>1,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'AnimCtg.delete_flg',
					'clm_show'=>1,
			),
			'update_user'=>array(
					'name'=>'更新者',
